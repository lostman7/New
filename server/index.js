...
const CONFIG_FILE = path.join(CONFIG_ROOT, 'ai-config.json');
...
function sanitizeAudioConfig(incoming, defaults) {
  const base = {
    preferredVoice: (defaults && defaults.preferredVoice) || null,
    autoReadResponses: Boolean(defaults && defaults.autoReadResponses),
  };
  if (!incoming || typeof incoming !== 'object') {
    return base;
  }
  const result = { ...base };
  if (typeof incoming.preferredVoice === 'string') {
    const trimmed = incoming.preferredVoice.trim();
    result.preferredVoice = trimmed.length > 0 ? trimmed : null;
  } else if (incoming.preferredVoice === null) {
    result.preferredVoice = null;
  }
  if (typeof incoming.autoReadResponses === 'boolean') {
    result.autoReadResponses = incoming.autoReadResponses;
  }
  return result;
}

function sanitizeSlotConfig(slotId, incoming, defaults) {
  const base = { ...defaults, slot: slotId };
  if (!incoming || typeof incoming !== 'object') {
    return base;
  }
  const result = { ...base };
  if (typeof incoming.name === 'string') {
    result.name = incoming.name.trim();
  }
  if (typeof incoming.persona === 'string') {
    result.persona = incoming.persona.trim();
  }
  if (typeof incoming.provider === 'string') {
    result.provider = incoming.provider.trim() || base.provider;
  }
  if (typeof incoming.apiKey === 'string') {
    result.apiKey = incoming.apiKey.trim();
  }
  if (typeof incoming.baseUrl === 'string') {
    result.baseUrl = incoming.baseUrl.trim();
  }
  if (typeof incoming.model === 'string') {
    result.model = incoming.model.trim();
  }
...
function sanitizeConfig(config) {
  ...
  const incomingSlots = (config && config.aiSlots) || {};
  const result = {
    version: 1,
    savedAt: (config && config.savedAt) || null,
    aiSlots: {
      ai1: sanitizeSlotConfig('ai1', incomingSlots.ai1 || incomingSlots['ai1'], defaults.aiSlots.ai1),
      ai2: sanitizeSlotConfig('ai2', incomingSlots.ai2 || incomingSlots['ai2'], defaults.aiSlots.ai2),
    },
    audio: sanitizeAudioConfig(config && config.audio, defaults.audio),
  };
  return result;
}

function normalizeRuntimeSlots(slots) {
  const config = loadAiConfig();
  const defaults = (config && config.aiSlots) || {};
  return slots.map((slot, index) => {
    const slotId = (slot && slot.slot) || `ai${index + 1}`;
    const fallback = defaults[slotId] || createDefaultConfig().aiSlots[slotId] || { slot: slotId };
    return sanitizeSlotConfig(slotId, slot, fallback);
  });
}
...
function loadAiConfig() {
  ...
  try {
    ...
    serverState.config = sanitizeConfig(parsed);
  } catch (err) {
    serverState.config = createDefaultConfig();
    serverState.config.savedAt = new Date().toISOString();
    fs.mkdirSync(CONFIG_ROOT, { recursive: true });
    fs.writeFileSync(CONFIG_FILE, JSON.stringify(serverState.config, null, 2));
  }
  return serverState.config;
}
...
function providerLabel(slotConfig) {
  if (!slotConfig || !slotConfig.provider) {
    return 'custom';
  }
...
function buildAiResponse(slotConfig, message) {
  ...
  const providerSummary = providerLabel(slotConfig);
  const endpointNote = slotConfig.baseUrl ? ` via ${slotConfig.baseUrl}` : '';
  const content = `${personaPrefix}Responding as ${slotConfig.name || 'AI'} (model: ${slotConfig.model || 'unspecified'} • ${providerSummary}${endpointNote}) to "${message}".${knowledgeSnippet}`;
  return {
...
function buildArenaExchange(aiSlots, seedMessage) {
  ...
  for (let turn = 0; turn < turns; turn += 1) {
    ...
    const knowledgeSnippet = sampleKnowledgeSnippet();
    const providerSummary = providerLabel(speaker);
    const endpointNote = speaker.baseUrl ? ` via ${speaker.baseUrl}` : '';
    const content = `${speaker.name || 'AI'} replying to ${listener && listener.name ? listener.name : 'their peer'} after hearing "${previousMessage}". ${personaHint} (provider: ${providerSummary}${endpointNote})${knowledgeSnippet}`;
...
const apiRoutes = {
  ...
  async '/api/chat'(req, res) {
    ...
    try {
      ...
      const config = loadAiConfig();
      const providedSlots = Array.isArray(payload.aiSlots) && payload.aiSlots.length > 0
        ? payload.aiSlots
        : Object.values((config && config.aiSlots) || {});
      const normalizedSlots = normalizeRuntimeSlots(providedSlots);

      let aiSlots = normalizedSlots;
      if (mode === 'standalone') {
        aiSlots = normalizedSlots.filter(slot => slot.slot === 'ai1');
        if (aiSlots.length === 0 && normalizedSlots.length > 0) {
          aiSlots = [normalizedSlots[0]];
        }
        aiSlots = aiSlots.map(slot => ({ ...slot, enabled: slot.enabled !== false }));
        aiSlots = aiSlots.filter(slot => slot.enabled !== false);
      } else if (mode === 'duet') {
        aiSlots = normalizedSlots.filter(slot => slot.enabled !== false);
        if (aiSlots.length === 0 && normalizedSlots.length > 0) {
          aiSlots = [normalizedSlots[0]];
        }
      } else if (mode === 'arena') {
      ...
      if (mode === 'arena' && aiSlots.length < 2) {
        throw new Error('Arena mode requires two enabled AI slots. Update your options to enable both AIs.');
      }

      if (aiSlots.length === 0) {
        throw new Error('No AI slots are enabled. Please configure your AI companions in Options.');
      }
      ...
      if (mode === 'arena') {
        ...
        entry = {
          ...
          mode,
          message,
          aiSlots: aiSlots.map(({ apiKey, ...rest }) => ({ ...rest })),
  ...
  async '/api/config'(req, res) {
    ...
    if (req.method === 'POST') {
      try {
        const body = await collectRequestBody(req);
        const payload = body ? JSON.parse(body) : {};
        let incomingSlots = payload.aiSlots;
        if (Array.isArray(incomingSlots)) {
          incomingSlots = incomingSlots.reduce((acc, slot, index) => {
            if (!slot || typeof slot !== 'object') return acc;
            const slotId = (typeof slot.slot === 'string' && slot.slot.trim().length > 0)
              ? slot.slot.trim()
              : `ai${index + 1}`;
            acc[slotId] = slot;
            return acc;
        ...
        const configPayload = {
          ...
          aiSlots: incomingSlots || payload.aiSlots || {},
          audio: payload.audio || {},
        };
        const config = persistAiConfig(configPayload);
        sendJSON(res, 200, { config });
      } catch (error) {
